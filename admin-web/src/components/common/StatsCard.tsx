...
export const StatsCard: React.FC<StatsCardProps> = ({
  ...
  return (
    <div className={`bg-white px-4 py-2 rounded-[20px] shadow-sm border border-slate-200 flex items-center gap-3 ${className}`}>
      <div className={`w-8 h-8 ${bgColor} ${iconColor} rounded-lg flex items-center justify-center shrink-0 [&_svg]:shrink-0`}>
        {icon}
      </div>
      <div className="min-w-0 flex-1">
        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tight truncate" title={label}>{label}</p>
