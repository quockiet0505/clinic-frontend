...
export function buildRescheduleDates(maxDaysAhead = 7): RescheduleDateOption[] {
...
export default function RescheduleDatePicker({ value, onChange, maxDaysAhead = 7 }: Props) {
  ...
  const selectableSet = useMemo(() => new Set(availableDates.map((d) => d.dateString)), [availableDates]);
  ...
  return (
    <div className="flex flex-col gap-3">
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1">Chỉ chọn trong vòng {maxDaysAhead} ngày tới (trừ cuối tuần)</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {quickDates.map((item) => (
          ...
          >
            <span className={`text-[15px] font-black ${value === item.dateString ? 'text-primary-600' : 'text-slate-800'}`}>
              {item.displayDate}
            </span>
            <span className={`text-[13px] font-medium mt-0.5 ${value === item.dateString ? 'text-primary-600' : 'text-slate-500'}`}>
              {item.dayOfWeek}
            </span>
          </button>
        ))}
        ...
        >
          ...
          <span className={`text-[13px] font-bold ${isCustomSelected ? 'text-primary-600' : 'text-slate-700'}`}>
      ...
      <Dialog open={calendarOpen} onOpenChange={setCalendarOpen}>
        <DialogContent
          ...
          className="max-w-[640px] w-[90vw] rounded-2xl p-0 gap-0 overflow-hidden bg-white"
        >
          <div className="flex items-center justify-between border-b border-slate-100 bg-white px-5 py-4 rounded-t-2xl">
            <DialogTitle className="text-lg font-bold text-slate-800 pr-4">
              Chọn ngày dời lịch
            </DialogTitle>
            <DialogClose className="shrink-0 rounded-full p-1.5 hover:bg-slate-100 transition">
          ...
          <div className="p-5">
            <div className="rounded-xl border border-slate-200 overflow-hidden">
              <div className="flex items-center justify-between bg-primary-500 text-white px-4 py-2">
                ...
                {allDays.map((date, idx) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={!selectable}
                      className={`h-11 flex items-center justify-center text-sm font-medium transition-all ${
                        selected
