...
const SOURCE_TABS = [
  ...
  { label: 'Khách vãng lai', value: 'WALK_IN' }
];
...
export default function AppointmentFilterBar({ search, setSearch, typeFilter, setTypeFilter, fromDate, toDate, setFromDate, setToDate }: Props) {
  return (
    <div className="flex flex-col xl:flex-row gap-4 bg-white p-4 rounded-[20px] border border-slate-200 shadow-sm shrink-0 items-center justify-between">
      ...
      {/* TABS LỌC NGUỒN */}
      <div className="flex bg-slate-50 p-1 rounded-2xl w-full xl:w-auto overflow-x-auto border border-slate-100">
        {SOURCE_TABS.map((tab) => (
          <button
            key={tab.value}
            onClick={() => setTypeFilter(tab.value)}
            className={`flex-1 xl:flex-none px-6 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 ease-out whitespace-nowrap cursor-pointer ${
