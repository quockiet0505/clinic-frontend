    ...
    <Dialog open={!!expertise} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden border-0 rounded-2xl shadow-2xl">
        ...
        <DialogFooter className="p-6 bg-white border-t border-slate-100 flex gap-3">
          ...
          <Button onClick={() => onSave(expertise.expertise_id, formData)} className="rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-md hover:shadow-lg text-white font-bold px-6 shadow-sm">Save Changes</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
