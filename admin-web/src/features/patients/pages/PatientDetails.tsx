    ...
    <div className="space-y-6 animate-in fade-in duration-500 h-[calc(100vh-6rem)] flex flex-col">
      <div className="max-w-[1000px] mx-auto w-full flex flex-col h-full space-y-6">
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Patient Profile Overview</h1>
            <p className="text-sm text-slate-500 mt-1">Viewing comprehensive demographics for <span className="font-bold text-blue-600">PAT-{id}</span></p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[32px] shadow-sm border border-slate-200 overflow-hidden shrink-0">
          <div className="bg-blue-600 p-6 flex flex-col sm:flex-row items-center sm:items-start gap-6">
            ...
            <div className="flex-1 w-full text-center sm:text-left text-white">
              <h2 className="text-3xl font-black">{patient.full_name}</h2>
              <div className="mt-1 flex flex-wrap items-center justify-center sm:justify-start gap-2">
                <span className="bg-white/20 text-white px-3 py-1 rounded-lg font-bold text-sm">PAT-{patient.patient_id}</span>
                <span className="border border-white/30 px-3 py-1 rounded-lg text-white font-medium text-sm">Active</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 bg-slate-50/50">
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-4">
                <UserCircle2 size={16} /> Personal Information
              </h3>
              <div className="grid grid-cols-3 gap-2 text-sm">
                <span className="font-bold text-slate-500">Gender/Age:</span>
                <span className="col-span-2 font-bold text-slate-900">{patient.gender}, {patient.age} years old ({patient.date_of_birth})</span>
            ...
            <div className="space-y-4">
              <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-4">
                <Activity size={16} /> Emergency & Vital Profile
              </h3>
              <div className="grid grid-cols-3 gap-2 text-sm">
                <span className="font-bold text-slate-500">Blood Type:</span>
                <span className="col-span-2 font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded-md w-max">{patient.blood_type}</span>
                ...
                <span className="font-bold text-slate-500 mt-2">Chronology:</span>
                <span className="col-span-2 font-bold text-slate-900 mt-2 leading-relaxed">{patient.chronic_diseases}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[32px] shadow-sm border border-slate-200 overflow-hidden flex-1 flex flex-col">
          <div className="p-5 bg-slate-50/50 border-b border-slate-100 flex items-center gap-2 shrink-0">
            ...
            <h2 className="font-black text-slate-800 uppercase text-[11px] tracking-widest">Recent Clinical Visits</h2>
          </div>
          
          <div className="p-0 overflow-y-auto custom-scrollbar flex-1 bg-white">
            <PatientHistoryTimeline visits={patient.recent_visits} />
          </div>
