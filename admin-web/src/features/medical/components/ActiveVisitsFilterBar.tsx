  ...
  return (
    <div className="flex flex-col xl:flex-row gap-3 items-center justify-between">
      <div className="hidden xl:flex items-center gap-1.5">
        <Activity size={14} className="text-slate-400" />
        <span className="text-xs font-semibold text-slate-500">Đang khám</span>
      </div>
      <div className="flex flex-col sm:flex-row items-center gap-2.5 w-full xl:w-auto">
        <SearchInput value={search} onChange={setSearch} placeholder="Tìm bệnh nhân..." className="w-full sm:w-48" />
        <DateRangeFilter from={fromDate} to={toDate} onChangeFrom={setFromDate} onChangeTo={setToDate} />
      </div>
    </div>
  );
