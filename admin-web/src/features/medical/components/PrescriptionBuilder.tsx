...
export default function PrescriptionBuilder() {
  ...
  const handleAdd = () => {
    if (med && dosage && qty) {
      setItems([...items, { id: Date.now(), medicine_id: 1, medication_name: med, dosage, quantity: Number(qty) }]);
    ...
    <div className="space-y-6">
      <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex flex-col xl:flex-row gap-4 items-end">
        <div className="w-full xl:flex-1 space-y-2">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Medication</label>
          <Input value={med} onChange={(e) => setMed(e.target.value)} className="h-11 rounded-xl bg-white" placeholder="Search medicine..." />
        </div>
        <div className="w-full xl:flex-1 space-y-2">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Dosage / Instructions</label>
        ...
        <div className="w-full xl:w-24 space-y-2">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Qty</label>
          <Input type="number" value={qty} onChange={(e) => setQty(e.target.value)} className="h-11 rounded-xl bg-white" placeholder="0" />
        </div>
        <Button onClick={handleAdd} className="h-11 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-md hover:shadow-lg text-white font-bold px-6 w-full xl:w-auto">
