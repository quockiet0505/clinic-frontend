      ...
      <Table>
        <TableHeader className="bg-slate-50">
          <TableRow className="h-14">
            ...
            <TableHead className="font-bold text-slate-600 text-[11px] uppercase text-right pr-8">Amount</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {data.map((exp) => (
            <TableRow key={exp.expense_id} className="hover:bg-slate-50/50 cursor-pointer transition-colors" onClick={() => onRowClick(exp)}>
              <TableCell className="px-8 py-4">
                <p className="font-bold text-slate-900">EXP-{exp.expense_id}</p>
                <p className="text-xs text-slate-500 font-medium">{exp.expense_date}</p>
              </TableCell>
              <TableCell><span className="font-bold text-slate-700">{exp.category_name}</span></TableCell>
