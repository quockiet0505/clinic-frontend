  ...
  return (
    <div className="bg-white rounded-[32px] shadow-sm border border-slate-200 flex-1 overflow-hidden">
      <Table>
        ...
        <TableBody>
          {data.map((log) => (
            <TableRow key={log.refund_id} className="hover:bg-slate-50/50">
              <TableCell className="px-8 py-4">
                <p className="font-bold text-slate-900">{log.refunded_at.split('T')[0]}</p>
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">REF-{log.refund_id}</p>
              </TableCell>
              <TableCell>
                <div className="flex items-center gap-1.5 font-bold text-blue-600">
                  <FileText size={14} /> BILL-{log.bill_id}
                </div>
              </TableCell>
              ...
              <TableCell className="text-right pr-8">
                <span className="text-lg font-black text-rose-600 flex items-center justify-end gap-1">
                  <Undo2 size={16} /> ${log.refund_amount.toFixed(2)}
                </span>
              </TableCell>
            </TableRow>
          ))}
