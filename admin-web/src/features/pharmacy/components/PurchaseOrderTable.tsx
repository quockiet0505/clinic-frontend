      ...
      <Table>
        ...
        <TableBody>
          {data.map((po: PurchaseOrder) => (
            <TableRow key={po.po_id} className="hover:bg-slate-50/50">
              <TableCell className="px-8 py-4">
                <p className="font-black text-slate-900">${po.total_amount.toFixed(2)}</p>
                <p className="text-[10px] text-slate-500 font-bold tracking-widest mt-0.5">PO-{po.po_id} • {po.order_date.split('T')[0]}</p>
              </TableCell>
              <TableCell>
              ...
              <TableCell className="text-right pr-8">
                {po.status === 'PENDING' ? (
                  <div className="flex justify-end gap-2">
                    <Button onClick={() => onMarkReceived(po.po_id)} variant="outline" size="sm" className="h-9 font-bold rounded-xl text-emerald-600 border-emerald-200 hover:bg-emerald-50">
                  ...
                  <Button variant="ghost" size="sm" className="h-9 font-bold text-slate-400">
