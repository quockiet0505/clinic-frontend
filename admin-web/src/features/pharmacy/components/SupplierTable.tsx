  ...
  return (
    <div className="bg-white rounded-[32px] shadow-sm border border-slate-200 flex-1 overflow-hidden">
      <Table>
        ...
        <TableBody>
          {data.map(sup => (
            <TableRow key={sup.supplier_id} className="hover:bg-slate-50/50">
              <TableCell className="px-8 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-slate-900">{sup.supplier_name}</p>
                    <p className="text-xs text-slate-500 font-medium mt-0.5 max-w-[200px] truncate">{sup.address}</p>
                  </div>
                </div>
              </TableCell>
              <TableCell>
                <p className="font-bold text-slate-800">{sup.contact_name}</p>
              ...
              <TableCell className="text-center">
                <Badge variant="outline" className={`font-bold border-0 px-2.5 py-1 ${sup.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
