  ...
  if (!data.length) {
    return (
      <div className="text-center py-12 bg-white rounded-2xl border border-slate-200">
        <p className="text-slate-500 font-medium">Không có đơn thuốc nào.</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 h-full overflow-auto custom-scrollbar">
      <Table className="min-w-[900px]">
        <TableHeader className="bg-slate-100/80 sticky top-0 z-10">
          <TableRow>
            <TableHead className="px-6 py-4 text-left font-semibold text-slate-700 w-[15%]">Mã Đơn</TableHead>
            <TableHead className="px-6 py-4 text-left font-semibold text-slate-700 w-[25%]">Bệnh nhân</TableHead>
            <TableHead className="px-6 py-4 text-left font-semibold text-slate-700 w-[25%]">Bác sĩ kê đơn</TableHead>
            ...
            <TableRow key={rx.prescriptionId} className="hover:bg-slate-50 border-b border-slate-100 transition-colors">
              <TableCell className="px-6 py-4">
                <div className="flex items-center gap-2 font-bold text-slate-700">
                  <FileText size={16} className="text-blue-500" /> #RX-{rx.prescriptionId}
                </div>
                <div className="text-xs text-slate-500 mt-1">{rx.createdAt}</div>
              </TableCell>
              <TableCell className="px-6 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-bold text-slate-800 text-sm">{rx.patientName}</p>
              ...
              <TableCell className="px-6 py-4">
                <Button onClick={() => onViewDetails(rx.prescriptionId)} variant="outline" className="h-8 px-4 rounded-xl text-xs font-bold border-blue-200 text-blue-600 hover:bg-blue-50 transition-all shadow-sm">
