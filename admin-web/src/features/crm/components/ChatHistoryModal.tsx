      ...
      <DialogContent className="sm:max-w-[600px] p-0 overflow-hidden border-0 rounded-[32px] shadow-2xl bg-slate-50">
        ...
        <div className="bg-blue-600 p-6 flex justify-between items-start">
          <div className="text-white">
            ...
            <DialogTitle className="text-xl font-black">Session #{session.sessionId}</DialogTitle>
            <DialogDescription className="text-blue-100 mt-1">
              User: <span className="font-bold text-white">{session.patientName || 'Guest'}</span>
            </DialogDescription>
          </div>
          <Button variant="ghost" size="icon" onClick={onClose} className="rounded-full h-10 w-10 text-white hover:bg-blue-700 cursor-pointer"><X size={20}/></Button>
        </div>
        ...
        <div className="p-6 space-y-4 h-[50vh] overflow-y-auto custom-scrollbar flex flex-col">
          {session.messages?.map((msg: any) => {
            ...
            return (
              <div key={msg.messageId} className={`flex gap-3 max-w-[85%] ${isBot ? 'self-start' : 'self-end flex-row-reverse'}`}>
