...
import { Pill, CalendarDays, UserRound, FileSignature, X, Printer, Download, Stethoscope, Loader2 } from 'lucide-react';
import { DialogContent, DialogHeader, DialogTitle, DialogClose } from '@/components/ui/dialog';
import { ClinicPdfLayout } from '@/components/common/ClinicPdfLayout';
import type { PdfTableRow } from '@/components/common/ClinicPdfLayout';
...
export const PrescriptionModalContent = ({ prescription }: { prescription: any }) => {
  ...
  const handlePrint = () => {
    printPdfLayout(pdfId, 'Chi tiết đơn thuốc');
  };
  ...
  const tableRows: PdfTableRow[] = (prescription.items ?? []).map((item: any, idx: number) => ({
    index: idx + 1,
    name: item.medicineName,
    detail: item.dosage || 'Theo hướng dẫn bác sĩ',
    quantity: `${item.quantity} ${item.unit || 'viên'}`,
  }));

  const issuedDate = new Date(prescription.createdAt || new Date()).toLocaleDateString('vi-VN');
