  ...
  const handleSend = async (textToSend: string) => {
    ...
    try {
      await chatbotApi.streamMessage(userMsg.text, (chunkText: string) => {
        setIsTyping(false); // Ẩn typing indicator ngay khi có token đầu tiên
      ...
      setMessages(prevMessages =>
        prevMessages.map(msg =>
          msg.id === aiMsgId
            ? { ...msg, text: 'Không thể kết nối tới trợ lý AI. Vui lòng kiểm tra AI service (port 8000) và thử lại.' }
            : msg
        )
      );
    }
              ...
              return (
                <div key={msg.id} className={`flex w-full ${msg.sender === 'AI' ? 'justify-start' : 'justify-end'}`}>
                  <div className={`flex max-w-[85%] gap-2 ${msg.sender === 'AI' ? 'flex-row' : 'flex-row-reverse'}`}>
                    ...
                    {/* Avatar */}
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-sm overflow-hidden ${msg.sender === 'AI' ? 'bg-white' : 'bg-primary-100 text-primary-600'
                      }`}>
                      {msg.sender === 'AI' ? (
